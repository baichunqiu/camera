...
public class BCamera extends CameraDevice.StateCallback implements ICamera {
    ...
    private ICameraListeren mCameraListeren;

    @Override
    public final void init(Activity activity, AutoFitTextureView textureView) {
    ...
    @Override
    public final void setCameraListeren(ICameraListeren mCameraListeren) {
    ...
    private void openCamera(int width, int height) {
        ...
        try {
            ...
            //打开相机
            manager.openCamera(mCameraId, this, null);
        } catch (CameraAccessException e) {
            ...
            mCameraDevice.createCaptureSession(Collections.singletonList(previewSurface),
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigureFailed(@NonNull CameraCaptureSession session) {
                            ...
                            if (null != mCameraListeren)
                                mCameraListeren.onCameraError(-1, "Camera Configure Failed.");
                        }
                    }, mBackgroundHandler);
        } catch (CameraAccessException e) {
            e.printStackTrace();
    ...
    protected List<Surface> buildPreSurface(PreType preType) {
        ...
        try {
            if (PreType.video == preType) {
                ...
                if (null == mImageReader) return null;
                mPreviewBuilder = mCameraDevice.createCaptureRequest(CameraDevice.TEMPLATE_PREVIEW);
                mPreviewBuilder.addTarget(previewSurface);
                surfaces.add(previewSurface);
    ...
    public void startRecord(VideoParam param) {
        if (null == mCameraDevice || null == param) {
            return;
        }
        try {
            closePreviewSession();
            setUpMediaRecorder(param);
            List<Surface> surfaces = buildPreSurface(PreType.video);
            if (null == surfaces) {
                return;
            }
    ...
    private static class BSurfaceTextureListener implements TextureView.SurfaceTextureListener {
        ...
        public void onSurfaceTextureAvailable(SurfaceTexture surfaceTexture,
            ...
            bCamera.openCamera(width, height);
        }
